...
const Header = () =>{
    ...
    async function hh(){
        await fetch(`http://localhost:1337/api/projects?${type.map(item=>`filters[type][$in]=${item}`).join("&")}&${status.map(item=>`filters[state][$in]=${item}`).join("&")}&${selectedCategories.map(item=>`filters[category][$in]=${item}`).join("&")}&populate=*`)
        .then((response)=>{
